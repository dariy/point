...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-11 (commit a1c4ab9). Confidence: 99%.
...
### Architecture
Point is a personal photo blog engine: it ingests raw media files and post metadata through its web interface, CLI, or built-in MCP server, processes them via automated thumbnail generation and EXIF extraction, and serves an interactive timeline, map, and media gallery backed by SQLite and a Model Context Protocol remote control interface. It is designed for self-hosted individual photographers and creators who want an immersive, single-container publishing platform with social cross-posting and full programmatic control via AI agents. Work flows from the CLI and HTTP transport layers into the core business logic, database models, and integrated MCP endpoint. The api package acts as the backend engine, managing SQLite persistence, media processing services, scheduling, and streamable-HTTP server endpoints that handle both web requests and AI client tool calls.

### Key modules
- `api/internal` — Persistence operations and schema management are concentrated in dedicated repository files backed by embedded SQL migrations from…
- `frontend/src/utils` — To keep interface interactions smooth and predictable, helper modules manage structural presentation boundaries in the browser
- `api/internal/services` — The subsystem bridges external API requests from routers and controllers with low-level data access layers
- `api/internal/models` — The data layer centers on structured representations of application state, defining entities across models alongside supplemental…
- `frontend` — The core application setup initializes through frontend/src/app.js, which serves as the entry point loaded directly by frontend/index.html
- `frontend/src/plugins` — The subsystem consists of 39 JavaScript files spanning 20 distinct feature directories, maintaining an architectural separation between…
- `frontend/src/api` — Every API module relies on frontend/src/api/client.js as a thin fetch wrapper that automatically handles JSON serialisation…
- `api/internal/api` — The subsystem organizes HTTP request handling and business logic routing through dedicated handler modules
- `frontend/src/components/shared` — The user interface primitives in this group are built upon a shared component foundation where Modal acts as the generic overlay container…
- `frontend/src/components` — The codebase establishes a unified component model through frontend/src/components/Component.js, which serves as the base class extended by…
...
- `frontend/src/pages/light/PostEditPage.js` — 11 bug fixes, last fix 8 days ago (bug magnet); 35 commits/90d
- `api/cmd/api/main.go` — 9 bug fixes, last fix 7 days ago (bug magnet); 56 commits/90d
- `frontend/src/components/light/PhotoLibraryPickerDialog.js` — 5 bug fixes, last fix 8 days ago (bug magnet); 12 commits/90d
- `frontend/src/pages/public/TagPage.js` — 6 bug fixes, last fix 13 days ago (bug magnet); 29 commits/90d
- `frontend/src/pages/public/PostPage.js` — 6 bug fixes, last fix 3 weeks ago (bug magnet); 18 commits/90d

### Code health
Three co-equal signals: defect risk 6.28/10 avg, hotspot health 4.02/10 (stable), worst `frontend/src/pages/public/HomePage.js` at 1.0/10 · maintainability 8.13/10 · performance risk 145 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `frontend/src/plugins/comments/CommentsAdminPage.js` — untested hotspot — impact −2.0
- `eslint.config.js` — untested hotspot — impact −2.0
- `frontend/src/components/public/PostContent.js` — untested hotspot — impact −2.0
