...
# Tests from test_api/test_admin_coverage.py
@pytest.fixture
async def admin_user_coverage(db: AsyncSession):
    """Create admin user for coverage tests."""
...
@pytest.fixture
def override_auth(admin_user_coverage):
    """Override auth dependency for specific tests."""
    app.dependency_overrides[get_current_user] = lambda: admin_user_coverage
    yield
    app.dependency_overrides.pop(get_current_user, None)

@pytest.mark.asyncio
async def test_admin_dashboard_with_data(client: AsyncClient, admin_user_coverage, db: AsyncSession, override_auth):
    """Test admin dashboard with various data present."""
    # Add a post and media to trigger stats
    post = Post(title="T", slug="s", content="C", status=PostStatus.PUBLISHED, author_id=admin_user_coverage.id, view_count=10)
...
@pytest.mark.asyncio
async def test_admin_posts_list_filter(client: AsyncClient, admin_user_coverage, db: AsyncSession, override_auth):
    """Test admin posts list with filter."""
    # Use very specific names to avoid matching sidebar 'Recent Posts' if they appear there
    p1 = Post(title="UniqueDraftTitle", slug="d", content="C", status=PostStatus.DRAFT, author_id=admin_user_coverage.id)
    p2 = Post(title="UniquePublishedTitle", slug="p", content="C", status=PostStatus.PUBLISHED, author_id=admin_user_coverage.id)
    db.add_all([p1, p2])
    await db.commit()
...
@pytest.mark.asyncio
async def test_admin_edit_post_not_found(client: AsyncClient, override_auth):
    """Test edit post 404."""
    resp = await client.get("/light/posts/999")
    assert resp.status_code == 404

@pytest.mark.asyncio
async def test_admin_tags_page_params(client: AsyncClient, db: AsyncSession, override_auth):
...
@pytest.mark.asyncio
async def test_admin_media_page_filter(client: AsyncClient, db: AsyncSession, override_auth):
...
@pytest.mark.asyncio
async def test_admin_system_page(client: AsyncClient, override_auth):
