...
func TestOfflineStats_DBError(t *testing.T) {
	...
	cacheSvc := services.NewCacheService(tmpDir)
	sh := NewSystemHandler(h.repo, h.mediaSvc, h.postSvc, h.settingsSvc, h.tagSvc, systemSvc, cacheSvc, nil, tmpDir, "1.0")
	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	rec := httptest.NewRecorder()
...
func TestOfflineStats_NonImageMedia(t *testing.T) {
	...
	sh := NewSystemHandler(h.repo, h.mediaSvc, h.postSvc, h.settingsSvc, h.tagSvc, systemSvc, cacheSvc, nil, tempDir, "1.0")
	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	rec := httptest.NewRecorder()
	err := sh.GetOfflineStats(e.NewContext(req, rec))
