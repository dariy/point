...
WHERE
    ...
    END)
    AND (? OR NOT EXISTS (
        SELECT 1 FROM post_tags pt JOIN ehp ON pt.tag_id = ehp.id WHERE pt.post_id = p.id
    ))
    AND (
...
LIMIT ? OFFSET ?`
	...
	rows, err := r.db.QueryContext(ctx, q,
		statusFilter, status, featuredFilter, includeDrafts, includeHidden, includeHidden,
