...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-18 (commit 808f5b4). Confidence: 100%.Scope: standard index · model content · full Git. Missing or unavailable evidence is not a negative finding. Machine-readable scope: `{"analysis": {"skipped": [], "unavailable": []}, "content_provenance": "model", "file_pages": {"configured_cap": null, "effective_cap": null, "eligible": null, "generated": null, "omitted": null}, "git_commit_cap": null, "git_history_coverage": null, "git_tier": "full", "provider": {"embedder": "ollama", "model": "gemini-3.5-flash-lite", "model_cost_possible": null, "name": "gemini", "reused": null}, "run_mode": "standard", "search": {"full_text": "unknown", "next_command": null, "semantic": "unknown"}, "upgrade": {"completed_stages": [], "next_stage": null, "retryable": null, "status": "unknown"}, "version": 1}`
...
### Architecture
Point is a personal photo blog engine that ingests raw media and metadata files through a self-hosted backend, processes them into optimized thumbnails and extracted EXIF data via Go services, and serves an interactive timeline, world map, and admin UI to visitors and MCP clients. Designed for photographers and self-hosters, the application consolidates media management, geo-tagging, comments, and automated social cross-posting into a single container deployment. The codebase combines two distinct architectural views to power its capabilities: a robust backend execution model and a reactive client-side presentation layer. The backend manages database persistence, background task scheduling, and an in-process Model Context Protocol (MCP) server that enables remote administration through AI clients.

### Key modules
- `api/internal` — The internal codebase is organized into specialized functional boundaries that manage everything from server configuration to protocol…
- `frontend/src/utils` — Operating directly against the Document Object Model, the subsystem provides structural encapsulation for viewport and overlay behaviours
- `api/internal/services` — The services layer orchestrates complex workflows across the application domains, balancing concerns between content publication, metadata…
- `api/internal/models` — The subsystem organizes data access into discrete functional domains handled by api/internal/repository/queries_posts.go…
- `frontend` — The subsystem hosts the entry point frontend/src/app.js, which is loaded directly by frontend/index.html to initialize the client…
- `frontend/src/plugins` — The plugins subsystem delegates detailed responsibilities to specialized sub-components
- `frontend/src/api` — At the architectural foundation lies frontend/src/api/client.js, a thin fetch wrapper that handles JSON serialisation and deserialisation…
- `api/internal/api` — The frontend API client layer is partitioned into specialized handler files that coordinate domain logic across posts, media…
- `frontend/src/components/shared` — The user-facing overlay system relies on a consistent inheritance model descending from Component
- `frontend/src/components` — The subsystem is organized around a class hierarchy anchored by the base Component pattern, extended across administrative sections, public…
...
- `frontend/src/pages/light/PostEditPage.js` — 11 bug fixes, last fix 2 weeks ago (bug magnet); 27 commits/90d
- `frontend/src/plugins/carousel/index.js` — 3 bug fixes, last fix today (bug magnet); 23 commits/90d
- `api/cmd/api/main.go` — 9 bug fixes, last fix 2 weeks ago (bug magnet); 43 commits/90d
- `frontend/test/CarouselStudioPage.test.js` — 3 bug fixes, last fix today (bug magnet); 19 commits/90d
- `frontend/src/components/light/PhotoLibraryPickerDialog.js` — 5 bug fixes, last fix 2 weeks ago (bug magnet); 13 commits/90d

### Code health
Three co-equal signals: code health 6.04/10 avg (Fair), hotspot health 4.33/10 (stable), worst `api/internal/services/post_publish.go` at 1.0/10 · maintainability 7.01/10 · performance risk 153 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
...
- `frontend/src/components/light/VisualEditor.js` — god class (VisualEditor) — impact −2.0
- `frontend/src/components/shared/MediaViewer.js` — god class (MediaViewer) — impact −1.9
- `frontend/src/utils/icons.js` — change entropy — impact −1.8
