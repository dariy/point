 ...
 * budget and the properties panel becomes a bottom sheet below 64em. Two of
 * those numbers cannot stay in the stylesheet alone — the page has to ask
 * "sheet or rail?" before it renders (a sheet always opens closed), and the
 * zoom control has to clamp what it writes into `--carousel-stage-zoom`.
 *
 ...
 *  the page asks. Mirrors `@media (max-width: 64em)` in `carousel.css`, which
...
/** Where the panel's open/closed choice is remembered. Read only on a wide
 *  viewport — a sheet over the stage always opens closed, the way
 *  `PostEditPage._readDetailsPref` treats the editor's Details rail. */
...
/**
 * Is this viewport wide enough for the panel to be a rail rather than a sheet?
 * True where there is no `matchMedia` to ask (a test, or SSR): the rail is the
 * layout that works without JS.
 *
...
/**
 * The panel's initial open state: the remembered choice on a wide viewport,
 * always closed on a narrow one. Defaults to open — the panel holds the
 * controls the studio is for, so a first visit should see them.
 *
 * @param {{matchMedia?: (q: string) => {matches: boolean}}} [win]
