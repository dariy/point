...
describe('GestureController', () => {
  ...
  test('should initialize listeners with passive: true', () => {
    const listeners = [];
    ...
    // We specifically want to verify touchmove is now passive
    const touchmove = listeners.find(l => l.event === 'touchmove');
    assert.strictEqual(touchmove.options.passive, true);
  });
});
