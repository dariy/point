...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-14 (commit 09db2fb). Confidence: 99%.
...
### Architecture
Point is a personal photo blog engine that ingests raw media files, EXIF data, and post drafts through an in-process MCP server or admin UI, processes them into optimized thumbnails and geo-tagged timeline entries, and serves a media-centric web interface with interactive mapping and comment integrations. Designed for self-hosters and photographers, it provides an immersive viewing experience, Instagram cross-posting, and full remote control via Model Context Protocol clients. The system is structured around the api package, which acts as the core backend engine handling HTTP routing, database interactions via SQLite/Postgres, media asset generation, scheduling, and the built-in MCP server. Work moves from frontend client interactions—powered by centralized state stores and API client modules—into backend controllers and background services, such as the media processor and post scheduler.

### Key modules
- `api/internal` — The internal logic is partitioned into dedicated conceptual domains that manage distinct operational boundaries
- `frontend/src/utils` — The utility layer provides foundational data manipulation capabilities through a suite of pure and DOM-aware helpers
- `api/internal/services` — The codebase within api/internal/services is structured around distinct business domains handled by specialized files
- `api/internal/models` — The subsystem centers around api/internal/repository/db.go, which manages underlying database connections and initialization routines
- `frontend` — The initialization sequence begins in frontend/src/app.js, which is loaded directly by the browser via frontend/index.html
- `frontend/src/plugins` — The subsystem is organized around a collection of independent JavaScript features that integrate directly into the core application flow…
- `frontend/src/api` — The foundation of backend communication rests on frontend/src/api/client.js, a thin fetch wrapper that all domain-specific API modules…
- `api/internal/api` — The subsystem is structured around specialized handlers that parse incoming HTTP requests, validate input payload structures, and invoke…
- `frontend/src/components/shared` — The architecture of these shared elements builds upon a unified base component model
- `frontend/src/components` — The subsystem revolves around a shared object-oriented component foundation where specialized widgets and layout wrappers extend base…
...
- `frontend/src/pages/light/PostEditPage.js` — 11 bug fixes, last fix 12 days ago (bug magnet); 27 commits/90d
- `api/cmd/api/main.go` — 9 bug fixes, last fix 11 days ago (bug magnet); 50 commits/90d
- `frontend/src/components/light/PhotoLibraryPickerDialog.js` — 5 bug fixes, last fix 11 days ago (bug magnet); 13 commits/90d
- `frontend/src/pages/public/TagPage.js` — 6 bug fixes, last fix 2 weeks ago (bug magnet); 24 commits/90d
- `frontend/src/pages/public/PostPage.js` — 6 bug fixes, last fix 3 weeks ago (bug magnet); 19 commits/90d

### Code health
Three co-equal signals: defect risk 6.16/10 avg, hotspot health 3.86/10 (stable), worst `frontend/src/pages/public/HomePage.js` at 1.0/10 · maintainability 8.12/10 · performance risk 143 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `frontend/src/utils/icons.js` — untested hotspot — impact −2.0
- `frontend/src/plugins/slideshow/Slideshow.js` — untested hotspot — impact −2.0
- `frontend/src/plugins/comments/CommentsAdminPage.js` — untested hotspot — impact −2.0
- `frontend/src/plugins/breadcrumbs/Breadcrumbs.js` — untested hotspot — impact −2.0
- `frontend/src/pages/public/TagPage.js` — untested hotspot — impact −2.0
