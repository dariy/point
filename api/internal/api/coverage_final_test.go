...
func TestSystemHandler_GetLogs_ManyLines(t *testing.T) {
	...
	sh := NewSystemHandler(h.repo, h.mediaSvc, h.postSvc, h.settingsSvc, h.tagSvc, systemSvc, dataDir, "1.0")
	e := echo.New()
...
func TestOfflineStats_NonImageMedia(t *testing.T) {
	...
	`)
	systemSvc := services.NewSystemService(h.repo, t.TempDir())
	sh := NewSystemHandler(h.repo, h.mediaSvc, h.postSvc, h.settingsSvc, h.tagSvc, systemSvc, t.TempDir(), "1.0")
	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	rec := httptest.NewRecorder()
