...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-16 (commit 06e831b). Confidence: 99%.
...
### Architecture
Point is a personal photo blog engine: it ingests raw media files and post metadata through a command-line interface or an in-process MCP server, processes them via automated media pipelines for thumbnail generation and EXIF extraction, and serves an interactive timeline, map, and immersive gallery view via a unified backend and web UI. It is designed for photographers and creators who want self-hosted, media-centric publishing with robust geo-tagging, scheduling, and remote-control automation. The system is organized around a unified api package that houses the core Go backend services, database management, and the Model Context Protocol server. Work flows from the command-line and HTTP entry points through repository layers down to the SQLite database, while background schedulers handle timed publishing.
...
- `frontend/src/utils` — The foundation of the utility tier relies on general-purpose manipulation routines and presentation formatters
- `api/internal/services` — The subsystem manages central domain entities such as posts, tags, media assets, and system configurations through specialized components…
- `api/internal/models` — The subsystem defines core domain entities and specialised input parameters that govern data shape across the storage boundary
- `frontend` — The subsystem centers around frontend/src/app.js, which serves as the primary entry point for browser execution
- `frontend/src/plugins` — The plugins subsystem delegates detailed responsibilities to specialized sub-components
- `frontend/src/api` — At the foundation of the subsystem sits frontend/src/api/client.js, a thin wrapper around the native fetch API
- `api/internal/api` — The subsystem divides its interface responsibilities across a collection of focused handler modules, mapping concrete domain slices to…
- `frontend/src/components/shared` — The subsystem builds upon a common component foundation to establish consistent patterns across overlay interfaces and display elements
- `frontend/src/components` — The component model rests upon frontend/src/components/Component.js, which supplies the root Component class extended by core UI elements…
...
- `frontend/src/pages/light/PostEditPage.js` — 11 bug fixes, last fix 2 weeks ago (bug magnet); 25 commits/90d
- `api/cmd/api/main.go` — 9 bug fixes, last fix 13 days ago (bug magnet); 48 commits/90d
- `frontend/src/components/light/PhotoLibraryPickerDialog.js` — 5 bug fixes, last fix 13 days ago (bug magnet); 13 commits/90d
- `frontend/src/pages/public/TagPage.js` — 6 bug fixes, last fix 3 weeks ago (bug magnet); 23 commits/90d
- `frontend/src/pages/public/PostPage.js` — 6 bug fixes, last fix 3 weeks ago (bug magnet); 17 commits/90d

### Code health
Three co-equal signals: defect risk 6.28/10 avg, hotspot health 3.93/10 (stable), worst `frontend/src/pages/public/HomePage.js` at 1.0/10 · maintainability 8.12/10 · performance risk 149 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `scripts/check.sh` — untested hotspot — impact −2.0
